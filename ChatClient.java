	...
	public static void who() {
		...
		try {
			...
			try {
				...
				if (people.length == 1)
					ChatClient.text.append("You are alone.\n");
				else
					ChatClient.text.append(people.length + " people in this chat:\n");
				for (String p : people) {
					ChatClient.text.append(p + "\n");
				}
			} catch (RemoteException e) {
				System.out.println(
						"A RemoteException occurred when getting list of people from ChatServer: " + e.getMessage());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
